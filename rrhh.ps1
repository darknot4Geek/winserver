...
$excelFilePath = "C:\Users\Administrador\Desktop\RRHH.xlsx"

# Cargar el archivo Excel
$usersData = Import-Excel -Path $excelFilePath

# Establecer la contraseña predeterminada
$password = ConvertTo-SecureString "H1ztegia." -AsPlainText -Force

# Especificar la OU de destino
$targetOU = "OU=RRHH,DC=CiberDev,DC=local"

# Iterar a través de cada fila en el archivo Excel
foreach ($user in $usersData) {
    try {
        # Construir el nombre de usuario
        $username ="rh." + ($user.Izena.Substring(0, 3) + $user.Abizena.Substring(0, 4)).ToLower()
        ...
        Add-ADGroupMember -Identity "rrhhgroup" -Members $username
