...
$password = ConvertTo-SecureString "Usu4ariox." -AsPlainText -Force

# Especificar la OU de destino
$targetOU = "OU=ITjuniors,DC=CiberDev,DC=local"
...
foreach ($user in $usersData) {
    try {
        ...
        Add-ADGroupMember -Identity "ITjuniorgroup" -Members $username
